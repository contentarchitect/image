(function (global, factory) {
	...
	(global = typeof globalThis !== 'undefined' ? globalThis : global || self, global.Image = factory(global.ContentArchitect));
	...
	var script = {
		props: ['value'],
		components: {
			[editor.UiButton.name]: editor.UiButton,
		},
		directives: {
			SelectImage: editor.SelectImage,
	...
	var script$1 = {
		components: { CssGrid: editor.CssGrid, "v-button": editor.Button },
		props: ['value'],
		data () {
			return {
	...
	class Image extends editor.Block {
		...
		toHTML () {
			...
			this.images.forEach(image => {
				...
				Object.keys(image.data).forEach(key => {
					data.push(`data-${editor.Util.toKebabCase(key)}="${image.data[key]}"`);
				});
